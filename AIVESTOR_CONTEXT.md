...
| 平台代付 | 系统DeepSeek Key（`SYSTEM_DEEPSEEK_API_KEY`），手机号限额1000万tokens |
...
### free_quota_usage
```
id UUID PK | user_id FK | phone VARCHAR(20) UNIQUE
tokens_used BIGINT DEFAULT 0 | tokens_limit BIGINT DEFAULT 10000000
created_at | updated_at
```
...
### V2（2026-05-18）
知识库Chunking+向量检索、认知进化模块、投资决策中心（魔鬼代言人/行业外视角/历史镜像）、多文件上传、SKILL市场（20官方+自建）、认知进化深度功能、投后管理
...
| FREE_QUOTA_TOKENS | Vercel 环境变量（默认10000000）|
| 测试账号 | test@example.com / test12345 |
