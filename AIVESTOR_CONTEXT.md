...
| SSR 改造 | `/skills/page.tsx` 改为 async Server Component：服务端直接查 `skill_catalog`（+登录态自建 SKILL），`export const metadata`（SEO title/description），H1「SKILL 市场 / 一级股权投资分析框架库」+ 副标题；爬虫可读 SKILL 名称 |
...
P1 进度：
- #09 SKILL 市场公开页（未登录可访问的官方框架页）+ Landing Page SKILL 预览联动 ✅（2026-05-26，V2.9）
