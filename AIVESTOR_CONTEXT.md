...
> 最后更新：2026年5月26日（第七次更新 · GEO P0完成）
...
| **当前阶段** | V2.5 上线前安全加固完成，GEO P0开发任务完成，内测启动中 |
...
### V2.11（2026-05-27）项目列表 & 知识库搜索筛选
| 模块 | 说明 |
|------|------|
| 项目列表筛选 | `/projects` SSR 接 `searchParams`：`search`（防抖 300ms，匹配 `name` + `judgment_points::text`）、`stage`（融资轮次，下拉值取自用户自有项目 DISTINCT）、`process_stage`（`ALL_STAGES` 白名单）、`outcome`（pending/invested/passed/exited_profit/exited_loss 白名单）、`sort`（created_desc / updated_desc）。所有过滤条件参数化拼接 WHERE。新增 `ProjectFilters.tsx` 客户端组件，状态写入 URL query string，刷新保留。 |
| 项目 API | `GET /api/projects` 透传上述同名参数，复用同样的白名单校验与参数化 SQL。 |
| 知识库筛选 | `/knowledge` 顶部搜索栏旁新增条目类型 chip 多选（manual / conversation_digest / document_chunk / industry / project / thesis / prediction / chunk）+ 来源类型单选下拉（manual / document / report）。无搜索词时按 `created_at` 排序，有搜索词走原 `/api/knowledge/search`。 |
| 知识库 API | `GET /api/knowledge` 接收 `entry_type`（逗号分隔）、`source_type`；`POST /api/knowledge/search` 接收 `entry_type[]`、`source_type`，并在 FTS / 向量子查询的 WHERE 前置过滤；当过滤至非 document 时跳过 `document_chunks` 检索。 |
| 档案页筛选 | `/archive` SSR 接 `searchParams`：`search` / `process_stage` / `outcome` / `sort`（默认 `updated_desc`），白名单 + 参数化 SQL 与 `/projects` 一致；新增 `ArchiveFilters.tsx` 客户端组件（仿 `ProjectFilters`），状态写入 URL。空态分「无匹配」/「无档案」两种。 |
| 项目 API 扩展 | `GET /api/projects` 同时返回 `file_count` / `report_count`（用 `documents` / `reports` 子查询聚合），供档案卡片复用，避免新增接口。 |

### V2.10（2026-05-26）项目简要分析（快速入库评估）
| 模块 | 说明 |
|------|------|
| API | `POST /api/projects/[id]/brief-analysis`：无 3 条判断门槛、不强制 BP；聚合项目基本信息 + 已记录判断 + 知识库 FTS（用 `search_vector` 检 5 条相关）+ 投资人画像（`injectProfile`），调 `streamChat` 流式输出，`streamTextResponse` 入库到 `reports`，title 前缀「【简要分析】」 |
| 页面 | `/projects/[id]/brief-analysis`：SSR 拿项目名 + 客户端 `BriefAnalysisClient`（idle/generating/done/error 四态），ReactMarkdown 渲染（rehype-sanitize） |
| 项目页入口 | `ProjectDetail.tsx` 在「生成分析报告」旁加 Link「简要分析」（次按钮），下方加一句提示「还没填判断？试试简要分析」 |
| 档案角标 | `ArchiveTabs.reportBadge` 增加 `brief` kind，灰色徽章（`bg-slate-100 text-slate-600 ring-slate-200`） |
| 升级路径 | 完成后引导回 `/projects/[id]` 追加判断 → 生成深度分析报告；两份报告同时保存在档案，互不覆盖 |

### V2.9（2026-05-26）GEO P1 #09 — SKILL 公开页 + Landing 联动
| 模块 | 说明 |
|------|------|
| 公开访问 | middleware.ts matcher 移除 `/skills`，未登录可浏览全部官方 SKILL（`(app)/layout.tsx` 已对无 session 渲染纯净内容） |
| SSR 改造 | `/skills/page.tsx` 改为 async Server Component：服务端直接查 `skill_catalog`（+登录态自建 SKILL），`export const metadata`（SEO title/description），H1「SKILL 广场 / 一级股权投资分析框架库」+ 副标题；爬虫可读 SKILL 名称（命名统一用「SKILL 广场」） |
| 交互拆分 | 新增 `src/components/skills/SkillsClient.tsx`（"use client"，按 props 初始化）；未登录隐藏 创建/导入/「我的 SKILL」tab，卡片按钮「登录后使用」（橙色）跳 `/login?callbackUrl=/skills`；已登录行为不变（按钮文案改为「使用 →」） |
| Landing 联动 | `LandingPage.tsx` 在 FAQ 与 CTA 间新增「专业分析框架，开箱即用」Section（4 卡片预览 + 「查看全部20个分析框架 →」链接到 /skills） |
| 说明 | 任务文档写的登录路由 `/auth/signin` 在本项目不存在，实际登录页为 `/login`（NextAuth `pages.signIn`），已统一用 `/login?callbackUrl=/skills` |
...
P0 任务全部完成：
- #01 GitHub README 改造 ✅
- #02 Landing Page FAQ 模块 ✅
- #03 注册即刻官方账号 ⬜ 非开发任务，待创始人操作
- #04 Landing Page Schema.org 结构化标记 ✅
- #05 清理硬编码敏感信息 ✅（V2.5已完成）

P1 进度：
- #09 SKILL 广场公开页（未登录可访问的官方框架页）+ Landing Page SKILL 预览联动 ✅（2026-05-26，V2.9）
